...
- Postings viewer (`apps/web/src/components/PostingViewer.tsx`) with:
  ...
  - **sticky toolbar** (below the nav): row 1 — source, **rank status**, **archive**, sort, min score; row 2 — search (full width); **rows per page** (10 / 20 / 50 / 100, default 20, persisted in `localStorage`), **Select all visible**, bulk **Archive** split button, **Score selected** / **Delete selected** — stays visible while scrolling the list
  - **paginated list** via `postings.listPage` with **page navigation** (prev/next, page X of Y) and **rows per page** at the bottom; header shows matching count vs **active** database total
  - postings shown as a **list** (`PostingTable.tsx`): compact meta row (color-coded score, styled external title link, source, location, ranked/discovered/**archived**, actions), **server-truncated** description preview (~140 chars) with **More** / **Less** (lazy `postings.getDescription`), ranking details: **compact rubric strip** from `dimensionScoresJson`, **matched / unmet / red flag** pills from list payload, **Full scores** / **Summary** (lazy `ranking.getLatestReasoning` — full GFM table + details)
  - filters: `postings.listPage` supports `archiveVisibility` (`active` default | `archived` | `good` | `bad`), optional `company` (exact employer name), `rankStatus` (`ranked` | `unranked`), text search, source, `minScore`, and sort (`scoreDesc`, `rankedAtDesc`, `discoveredAtDesc`, `postedAtDesc`, `archivedAtDesc` when viewing archives). Free-text search scans full descriptions server-side but returns truncated snippets on the wire (slower on large tables).
  - per-item actions on **active** rows (`View`, **`Score`**, **Archive good** / **Archive bad**, **`Ask`**, **`Cover letter`**, `Delete`); on **archived** rows (`View`, **Unarchive**, **Mark good** / **Mark bad**, link to **N active from this employer**, `Delete`), multi-select checkboxes (larger click targets), bulk archive / `Score selected` / `Delete selected`, and `Clear All`
...
- `posting_cover_letter_outlines`: per-posting cover letter outline versions (`userMessage`, `outline`, optional `revisedFromId` parent link, `providerKey`, `model`, `status`, optional `errorMessage`, `createdAt`); deleted when the posting is removed
...
- `api.postingArchive.archive` (soft archive with `good` | `bad` label)
- `api.postingArchive.unarchive`
- `api.postingArchive.updateArchiveLabel`
- `api.postingArchive.countByCompanyActive`
- `api.postingArchive.countActiveForCompanies` (batch active counts for archived list employer links)
- `api.runs.list`
- `api.runs.get`
- `api.runs.trigger`
...
`VITE_WORKER_TRIGGER_URL`: URL for **Trigger now**, Cursor **Score** (`POST …/rank-posting`), Cursor **Ask** (`POST …/ask-posting`, `GET …/ask-logs`), and Cursor **Cover letter** (`POST …/cover-letter-outline`). Must match `WORKER_HTTP_TRIGGER_PORT`. Set in Settings (seeded from system defaults on first run), `.env.local`, or Vite (`VITE_*` wins in the browser when set at build/dev time). There is no hardcoded URL fallback in the web app.
...
- `WORKER_CHROME_USER_DATA_DIR` (optional; default empty → `~/.config/job-bot/chrome-profile`, or `chrome-profile-<WORKER_ID>` when `WORKER_ID` ≠ `default`): Chrome profile directory (cookies, LinkedIn `li_at` session). Relative paths resolve under `apps/worker`. **Contains session secrets** — do not copy or commit. Delete the folder to force a fresh LinkedIn login.
...
- `WORKER_AUTO_CLEANUP_CHROME` (default: `true`; stops the Chrome **process** after each LinkedIn run; cookies remain in the profile. Set `0` to keep the browser process running between runs while debugging)

**LinkedIn session (one-time setup):**

1. Enable Chrome (`WORKER_USE_CHROME=1`) and use a visible window (`WORKER_CHROME_HEADLESS=0`, or `npm run dev:all`).
2. Queue a LinkedIn scrape; sign in manually in the Chrome window (or set `LINKEDIN_USER` / `LINKEDIN_PASS` once).
3. Complete any CAPTCHA or security checkpoint in the browser if LinkedIn shows one.
4. Later runs reuse the same profile even when Chrome is restarted each run. CAPTCHA can still appear; persistent login reduces how often you need credentials.
- `WORKER_HTTP_TRIGGER_PORT` (optional): when set (e.g. `3999`), the worker listens on `127.0.0.1` for `GET /scheduler` (JSON status for the dashboard), `POST /trigger`, `POST /rank-posting` / `POST /rank-postings` (manual Cursor CLI score from the Postings page), `POST /ask-posting` and `GET /ask-logs?askRunId=…` (Cursor CLI Q&A from the Postings Ask panel), `POST /cover-letter-outline` (Cursor CLI cover letter outlines from the Postings Cover letter panel), and `POST /ingest-posting` (browser extension / manual capture ingest). Root `npm run dev:all` sets `3999` on the worker leg. These routes are **dev-only**: unauthenticated, bound to `127.0.0.1`, with CORS `*` for local tools.
...
- **Salary** — taken only from (1) the expanded “About the job” / legal pay text in the detail pane (regex patterns include `$90,000-120,000 base salary` style ranges where only the low bound has `$`), then (2) a short salary chip **inside that job’s list card**. It does **not** walk every `span`/`p`/`div` on the page (which previously reused one sidebar salary for unrelated postings).
...
**Cover letter outline (Postings Cover letter panel):** **`POST /cover-letter-outline`** with `{ postingId, userMessage, providerKey, model }`. The worker loads cover letter context from Convex (including prior completed versions), runs Cursor CLI, and persists via `postingCoverLetters.saveTurn`. Empty `userMessage` uses a default initial prompt.
...
**Postings page → Cover letter:** expand **Cover letter** on a row. Versions appear at the top in chronological order (oldest first, newest at the bottom; newest expanded by default); provider, model, and **Generate outline** / **New outline** form at the bottom. Click **Revise** on a version to edit that outline specifically; revisions link to the parent via `revisedFromId` and show lineage in the UI. **OpenAI** uses `api.postingCoverLetters.generateHttp`; **Cursor** uses `POST /cover-letter-outline` with optional `revisedFromId`. Defaults reuse Ask settings (`LLM_QA_PROVIDER` / `LLM_QA_MODEL`).
